...
app.post('/get_trending_posts',async (req,res)=>{
  const Data=await PostImage.find({}).sort({shared:-1,likes:-1,comments:-1})
  res.json(Data);
})
...
app.post('/get_trending_stories',async (req,res)=>{
  const category=req.body.category;
  const Data=await Other.find({category:category}).sort({shared:-1,likes:-1,comments:-1})
  res.json(Data);
...
app.post('/like',async (req,res)=>{
  ...
  if(isliked){
    ...
    user.notifications.push({name:from.name,text:"liked your post",profimg:from.profImg,category:"image",postid:post._id,from:from.email,time:new Date()})
    }
    res.json({yes:true,number:post.likes.size})
  }
  user.save()
...
app.post('/other_like',async (req,res)=>{
  ...
  if(isliked){
    ...
    if(user.email!=from.email){
    user.newnotification=true;
    user.notifications.push({name:from.name,text:"liked your post",profimg:from.profImg,category:post.category,from:from.email,postid:post._id,time:new Date()})
    }
    res.json({yes:true,number:post.likes.size})
  }
  user.save();
...
app.post('/profile_follow',async (req,res)=>{
  ...
  if(isfollowing){
    ...
    user.newnotification=true;
    user.notifications.push({name:follower.name,text:"started following you",profimg:follower.profImg,category:"follow",from:follower.email,time:new Date()})
...
app.post('/delete',async (req,res)=>{
  ...
  const user=await User.findOne({email:email})
  const index=user.notifications.findIndex(items=>items.postid==item.postid);
  if(index>-1){
